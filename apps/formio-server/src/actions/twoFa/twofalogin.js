...
const LOG_EVENT = '2FA Login Action';
...
module.exports = router => {
  ...
  /**
   * TwoFaLoginAction class.
   *   This class is used to create the 2FA action.
   */
  class TwoFaLoginAction extends Action {
    ...
    /**
     * Authenticate with Form.io using 2FA
     *
    ...
    resolve(handler, method, req, res, next) {
      debug('Starting 2FA Login');
      if (!hook.alter('resolve', true, this, handler, method, req, res)) {
        return next();
      }

      if (!req.token || !req.user) {
        return res.status(400).send('2FA Action is missing User.');
      }
      ...
      if (!is2FAEnabled) {
        return  res.status(400).send({
          message: '2FA Login is not enabled for current account.',
          twoFaEnabled: false
        });
      }

      if (!this.settings) {
        debug('Missed swttings');
        return res.status(400).send('Misconfigured 2FA Login Action.');
      }

      if (!this.settings.token) {
        debug('Token field setting missing');
        return res.status(400).send('2FA Login Action is missing Token Field setting.');
      }

      twoFa.secondFactorAuthentication(
          req,
          res,
        ...
        formio.auth.currentUser(req, res, (err) => {
          if (err) {
            log(req, ecode.auth.EAUTH, err);
            debug('2FA Login Action Current User Failed');
  ...
  // Return the TwoFaLoginAction.
  // Disable editing handler and method settings
  TwoFaLoginAction.access = {
    handler: false,
    method: false
  };

  return TwoFaLoginAction;
};
