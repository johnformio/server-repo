...
module.exports = app => routes => {
  const formResponseAlter = require('../../hooks/alter/formResponse')(app).alter;
  ...
  routes.after.push((req, res, next) => {
    if (['GET', 'INDEX'].includes(req.method)) {
      if (res.resource.item) {
        formResponseAlter(req.currentProject, res.resource.item);
      }
      if (res.resource.items) {
        _.map(res.resource.items, item => formResponseAlter(req.currentProject, item));
      }
    }
    next();
  });
