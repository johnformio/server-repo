...
module.exports = function(db, config, tools, done) {
  let _scriptFinished = false;
  let Local = Redis.createClient(config.redis.url, {max_attempts: 3});
  ...
  let getOldKeys = function(next) {
    ...
    let scan = function(cursor, type, keyFormat, cb) {
      if(cursor === '0' && started) {
        return cb();
      }
   ...
   */
  let addReqTypeToOldValue = function(value) {
    let oldValue = value.split(':');
    let delta = oldValue.pop(); // Remove the very last element which should always be the delta (total req time);
    let timestamp = oldValue.pop(); // Remove the next last element which should always be the event timestamp.
  ...
  let storeOldDataWithNewKeys = function(key, values, _callback) {
    ...
    let projectId = keyParts[1];
    let submissionType = keyParts[2];
    ...
    async.eachSeries(values, function(value, callback) {
      ...
      let curr = new Date(parseInt(valueParts[valueParts.length - 2]));
      let newKey = curr.getUTCFullYear() + ':' + curr.getUTCMonth() + ':' + curr.getUTCDate() + ':' + projectId + ':' + submissionType;

      // Update the new value to contain the request type.
      let newValue = addReqTypeToOldValue(value);
  ...
  let getOldValues = function(keys, next) {
  ...
  let markOldKeysForExpiration = function(keys, next) {
    let transaction = Local.multi();
